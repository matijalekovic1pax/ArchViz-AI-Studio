...
The script creates the `archviz_reports` database, `feedback_reports`, `feedback_activity`, `feedback_admins`, `app_generation_sessions`, and `app_request_logs` collections, the `feedback_snapshots` bucket, and seeds the initial feedback admin record for metadata. Admin API access is enforced in the Worker and is restricted to `matija.lekovic@1pax.com`. In production, generation/request logs use the same Appwrite backend as feedback reports; Supabase is retained only as a legacy fallback.
