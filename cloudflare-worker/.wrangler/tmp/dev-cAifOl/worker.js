...
// .wrangler/tmp/bundle-1uMpF0/checked-fetch.js
...
async function handleFetchUrl(request, env) {
  ...
  try {
    const resp = await fetchWithRetry(
      ...
      { maxRetries: 1, timeoutMs: 1e4, label: "URL fetch" }
    );
    if (!resp.ok) {
      return corsResponse(origin, {
        url: parsedUrl.href,
    ...
    const content = html.replace(/<script[^>]*>[\s\S]*?<\/script>/gi, "").replace(/<style[^>]*>[\s\S]*?<\/style>/gi, "").replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim().slice(0, 3e3);
    return corsResponse(origin, {
      url: parsedUrl.href,
      title,
      content,
...
__name(handleFetchUrl, "handleFetchUrl");
var worker_default = {
...
// .wrangler/tmp/bundle-1uMpF0/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-1uMpF0/middleware-loader.entry.ts
