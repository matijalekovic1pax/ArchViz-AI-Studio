...
export const APP_ASSISTANT_IMAGE_MODEL_RULES = [
  'Regular Nano Banana is the primary model for creating new photorealistic architectural renderings and render transformations. Prefer it for 3D Rendering, CAD to Render, Sketch to Render, Generate from Text, and other workflows where the user wants richer color, HDR feel, tone mapping, lighting, atmosphere, and photographic render polish.',
  'Regular Nano Banana is also strong for fast visual exploration and natural prompt following, but do not present it as the first choice for precise local edits to an already rendered image.',
...
export const APP_ASSISTANT_GUIDED_WORKFLOW_RULES = [
  ...
  'Visual Edit: if no clear selection exists, guide the user into Area/select mode before editing. Use AI selection when the target category is obvious, or lasso/brush when the user needs manual control. Ask what should change, what must stay locked, and whether a material/object/background reference image should be attached. Prefer ChatGPT Image Generation 2 for precise edits to finished renders or photos.',
...
export const APP_ASSISTANT_FEATURES: Record<GenerationMode, AppAssistantFeatureGuide> = {
  'generate-text': {
    ...
    specificGuidance: [
      ...
      'Use Regular Nano Banana when the user wants a new photorealistic rendering with stronger color, HDR-like tone, atmosphere, lighting, and render polish.',
  ...
  'render-3d': {
    ...
    specificGuidance: [
      ...
      'Run AI pre-processing during setup when the source contains skylights, ceiling frames, columns, trusses, glass, signage, stairs, or other details likely to need preservation.',
      ...
      'Use Regular Nano Banana as the primary recommendation for photorealistic 3D-to-render output, especially when the user wants stronger color, HDR/tone, lighting, atmosphere, and render polish.',
