...
export const APP_ASSISTANT_GLOBAL_RULES = [
  ...
  'Use available detection helpers during setup when they can reduce risk before the final run. Visual Edit auto-selection is reliable and may be offered or triggered when the target is one of its listed categories; use manual selection when the target is a specific instance rather than a category.',
...
/**
 * Intent to tool routing inside Visual Edit. Users describe an outcome, not a
 * tool; this maps the phrasings they actually use onto the tool that does the
 * job, so the assistant proposes one confident answer instead of listing the
 * whole panel.
 */
export const APP_ASSISTANT_VISUAL_EDIT_ROUTING = [
  'Route inside Visual Edit before answering: name one tool, say why, and set it. Do not list every tool back to the user.',
  '"Change the colour of X", "make X red", "recolour the arrows/seats/doors" -> Select tool, mark X, and put the request in the Edit Prompt verbatim.',
  '"Edit the floor", "change the flooring", "different wall finish", "make this concrete/oak/travertine" -> Material tool. Offer the material library or ask for a photo of the real finish. Auto selection with Floors or Walls usually gets the region in one step.',
  '"Remove this furniture", "get rid of that bin/sign/car", "delete this person" -> Remove tool with a selection around the subject; it also clears that subject\'s shadow and reflection and rebuilds what was behind it.',
  '"Remove all the cars", "no people anywhere", "clear every bin" -> Remove tool with Quick Remove target types and no selection, which sweeps the whole image.',
  '"Change the look of this man", "make these people realistic", "the figures look like mannequins/clay" -> People tool in Enhance mode. It redraws existing figures photorealistically while keeping their count, position, pose and clothing.',
  '"Add people here", "populate this corner", "fill the seating area" -> People tool in Repopulate mode with a selection.',
  '"The space looks empty", "populate the whole scene", "add people everywhere" -> People tool in Auto mode, which ignores selections and regenerates the entire frame.',
  '"Replace this chair with X", "swap the bench" -> Object tool in Replace mode, which keeps the original footprint, scale and perspective.',
  '"Add a bench/planter/kiosk here" -> Object tool in Place mode, which also rebuilds the surroundings around the new object.',
  '"Change the sky", "make it overcast/sunset" -> Sky tool.',
  '"Change what is outside the window", "different background behind the building" -> Background tool. The user selects the subject to keep; everything behind it is replaced.',
  '"Make it evening", "relight this area", "add sunlight from the left" -> Lighting tool for a local relight. For a whole-image time-of-day change on a model screenshot, 3D Rendering is the better feature.',
  '"Make the image wider", "show more of the room", "extend the frame" -> Extend tool. This is the one Visual Edit tool with its own image model choice.',
  '"Brighten it", "more contrast", "warmer", "crop to 16:9" -> Adjust tool, which applies locally without an AI round trip.',
  'If the request is about the whole image rather than part of it, and the source is a model screenshot rather than a finished render, route to 3D Rendering instead of Visual Edit.',
];
...
export const APP_ASSISTANT_GUIDED_WORKFLOW_RULES = [
  ...
  'Visual Edit: name the tool that matches the job before anything else, then get a selection in place. Offer Auto selection when the target is one of its categories, otherwise Rect, Brush or Lasso. Ask what should change and whether a material, object or background reference should be attached. Reassure the user that everything outside their selection is preserved exactly. Do not raise the image model: Visual Edit sets it automatically.',
  'Angle Change: confirm the intended full-frame angle and camera height before generating a new viewpoint.',
...
export const APP_ASSISTANT_FEATURES: Record<GenerationMode, AppAssistantFeatureGuide> = {
  ...
  'render-3d': {
    ...
    controls: ['Strict Realism/Enhance/Concept Push', 'source type', 'view type', 'style preset or style reference', 'background reference', 'Light Source grid', 'sun intensity', 'colour temperature', 'shadows', 'ambient fill and occlusion', 'time of day', 'atmosphere and mood', 'fog and bloom', 'scenery levels for people, vegetation and vehicles', 'Colour & Grade', 'Camera', 'Material Overrides', 'Source adherence', 'exclusions', 'aspect ratio', 'resolution'],
    specificGuidance: [
      ...
      'The 3D-to-render generation modes are Strict Realism, Enhance and Concept Push. If the user calls them styles, clarify that they are generation modes, not style presets.',
      'Concept Push is for a deliberately non-photographic concept image. It keeps the massing, camera angle and spatial organisation recognisable while allowing bolder interpretation of secondary forms, materials, lighting and entourage. Recommend it for early-stage or competition imagery, not for client-facing photoreal deliverables.',
      'Colour & Grade controls the image colour directly: dominant and accent palette anchors, white balance, saturation, contrast, and eight film grades (Straight, Neutral, Warm film, Cool editorial, Muted matte, High key, Deep contrast, Bleach bypass). Use this when the user asks for a colour direction, a brand palette or a photographic look rather than a change to the scene itself.',
      'Camera sets focal length in millimetres, camera height in centimetres, aperture as a real f-stop with matching depth of field, and exposure in stops. Use it when the user talks about lens choice, eye level, background blur or a brighter/darker exposure.',
      'Material Overrides assign a named part of the building to a material from the library, for example facade to travertine or floor to oak. These override whatever the style or source model implies. Use them when the user has a specified material board.',
      'Source adherence is a single dial from reinterpret freely to trace the model exactly. Raise it when geometry must survive, lower it when the user wants the model treated as a starting point.',
      'Exclusions is a plain-language list of things that must not appear, such as lens flare or visible watermarks.',
      'Scenery amounts for people, vegetation and vehicles are named levels rather than free sliders, because those are the steps the render actually distinguishes.',
      ...
      'If the user is working from a finished render/photo and asks for an isolated edit such as changing chair colour, wall paint, flooring, sky, signage or a furniture material, explain that 3D Rendering is the wrong feature and route them to Visual Edit, which preserves everything outside their selection.',
    ],
    ...
    suggestions: ['Walk me through 3D Rendering', 'Which generation mode should I use?', 'Give this render a warmer colour grade'],
  },
  'scene-compose': {
    mode: 'scene-compose',
    title: 'Scene Compose',
  ...
  'visual-edit': {
    mode: 'visual-edit',
    title: 'Visual Edit',
    summary: 'Change one part of an existing image while everything else stays exactly as it was.',
    bestFor: ['recolouring', 'material and finish swaps', 'removing objects or people', 'replacing objects', 'making rendered people realistic', 'adding people', 'sky and background replacement', 'relighting', 'extending the frame'],
    steps: [
      'Upload or select the image to edit.',
      'Pick the operation tool on the left that matches the job.',
      'Mark the area: Rect, Brush, Lasso, or Auto (pick a target such as Floors or Seating and it selects for you).',
      'Describe the change in the Edit Prompt field, or set the tool\'s own controls.',
      'Attach a material, object, or background reference when matching something real matters.',
      'Apply Edits.',
    ],
    controls: ['operation tool', 'selection mode (Rect/Brush/Lasso/Auto/Adjust/Erase)', 'Edit Prompt', 'feather', 'material preset or reference image', 'object placement mode', 'People mode', 'Quick Remove targets', 'lighting mode and Light Source grid', 'background reference', 'extend direction'],
    specificGuidance: [
      ...
      'Material restyles an existing surface. It takes either a preset from the material library or an uploaded reference photo of the real finish. The surface keeps its geometry, seams, edges and shading.',
      'Object has two modes. Replace swaps an existing object for another while keeping its footprint, scale and perspective. Place adds a new object and rebuilds the floor and background around it.',
      'People has three distinct modes. Enhance redraws people who are already in the image as photorealistic, keeping their exact count, position, pose, facing and clothing category, which is the right answer for clay, mannequin, white or low-poly figures. Repopulate adds new people to the selected area. Auto ignores any selection and populates the entire scene.',
      'Remove works two ways. With a selection it removes the single main subject at the centre of that selection along with its own shadow and reflection, and rebuilds what was behind it. With Quick Remove target types chosen and no selection, it removes every instance of those types across the whole image.',
      'Sky replaces the visible sky. Background replaces everything behind the selected subject, so the user selects the subject to keep, not the background to change.',
      'Lighting relights the whole scene. Light direction is set one of two ways: the Direction grid (Front, Back, Left, Right, never numeric values), or, with the grid turned off, by clicking the image to place numbered light-source markers on the actual door, window or lamp the light should come from. Markers only guide the model and never appear in the result. Recommend markers whenever the user names a specific opening, such as light coming through a particular door.',
      'Extend outpaints beyond the current frame and is the only Visual Edit tool with its own image model choice.',
      'Adjust applies tonal and colour post-production locally without going to the AI model, so it is instant and free of regeneration risk.',
      'If the user wants to add several separately referenced objects with placement control, redirect to Scene Compose.',
    ],
    watchOut: [
      'Leave a little room around the target rather than tracing it exactly; the edit follows real edges.',
      'People/Auto and Quick Remove regenerate the whole image, so unlike every other tool they do not preserve the rest of the frame.',
      'Use Scene Compose for many new referenced objects.',
    ],
    suggestions: ['Change the colour of something in this render', 'Remove this furniture', 'Make these rendered people look real'],
  },
  'angle-change': {
    mode: 'angle-change',
    title: 'Angle Change',
    summary: 'Generate one new camera viewpoint from an existing photo or render.',
    bestFor: ['subtle left/right full-frame camera shifts', 'higher or lower camera heights', 'alternate POVs', 'client viewpoint studies'],
    steps: [
      'Upload or select the source image on the canvas.',
      'Use the Frame Angle pad, sliders, or presets in the right panel.',
      'Set Angle Left/Right and camera height Lower/Higher.',
      'Generate one clean shifted view, then compare or download it from the left panel outputs.',
    ],
    controls: ['image preview overlay', 'angle/height pad', 'angle slider', 'height slider', 'frame angle presets', 'reset angle', 'generate new angle'],
    specificGuidance: [
      ...
      'Higher raises the camera and looks down on the subject (more ground, roofs and tops of objects); Lower drops the camera toward the ground and looks up (more sky or ceiling).',
      'Use Multi-Angle when the user needs a whole view set or grid instead of one new viewpoint.',
    ],
    watchOut: ['Results are strongest when the source shows enough geometry to infer depth.', 'This version is intentionally limited to angle and camera height only.'],
    suggestions: ['Make this a 24 degree right angle', 'Show this from a higher viewpoint', 'When should I use Multi-Angle instead?'],
  },
  exploded: {
    mode: 'exploded',
    title: 'Exploded View',
...
export function buildAppAssistantPrompt({
  ...
  return [
    ...
    'If you say you will switch modes, set a model, change settings, open a panel, run generation, or otherwise do something in the app, you must include the matching hidden <assistant_actions> JSON block. Never promise an app change in prose only.',
    ...
    '',
    'VISUAL EDIT TOOL ROUTING:',
    APP_ASSISTANT_VISUAL_EDIT_ROUTING.map((rule) => `- ${rule}`).join('\n'),
    '',
  ...
  switch (state.mode) {
    ...
    case 'angle-change':
      lines.push(
        ...
        `Camera height: ${wf.angleChangePitch} (positive = higher)`,
