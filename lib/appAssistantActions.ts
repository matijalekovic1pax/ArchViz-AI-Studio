...
export const buildAppAssistantActionContext = (
  ...
  return [
    ...
    `set_image_generation_model can switch the top-bar Image Model. Current: ${state.imageGenerationModel}. Allowed values: ${IMAGE_GENERATION_MODELS.join(', ')}. Regular Nano Banana is the default for flexible image work. ChatGPT Image Generation 2 is best for precision, preservation, and text-heavy images; if trial access is locked, the gateway will ask the user to unlock it from the top-bar model selector before generation can succeed.`,
